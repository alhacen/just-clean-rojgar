...
function select_year(year){
    for(i=1;i<=5;i++){
        _(i+"_sem_spi").style.display="none";
    }
    for(i=1;i<=year;i++){
        _(i+"_sem_spi").style.display="block";
    }
}
(function() {
    function toJSONString( form ) {
        var not_mandatory_field=["branch","alternative_contact_no","sem_1_1_spi","sem_1_2_spi","sem_2_1_spi","sem_2_2_spi","sem_3_1_spi","sem_3_2_spi","sem_4_1_spi","sem_4_2_spi","sem_5_1_spi","sem_5_2_spi"];
        var obj = {};
...
function sudoKill(){
    loading_scren_toggle()
    postData("http://127.0.0.1//upcjmi/server/auth.php?killSession&for=user")
        .then(function(data) {
...
function submit_auth_form(a){
    ...
    //console.log(data);
    //make_post_request("http://127.0.0.1//upcjmi/server/auth.php?"+a.innerHTML,data,check_auth)
    _("auth_loading_scren").style.display="block";
    _("auth_form_col").style.display="none";
    postData("http://127.0.0.1//upcjmi/server/auth.php?"+a.innerHTML, data)
        .then(function(data) {
            console.log(data);
            if(data.logged==1){
...
function check_user_status(){
    if(window.location.pathname=="/student/"){
        _("auth_form_col").style.display="none";
        loading_scren_toggle()
        postData("http://127.0.0.1//upcjmi/server/auth.php?status")
            .then(
                function(data) {
        ...
        postData("http://127.0.0.1//upcjmi/server/exe.php?fetch_me")
                    .then(function(data) {
                        if(data.data!="auth failed"){
                            loading_scren_toggle()
                            var private_data=["applied_for","img","profile_complete"];
...
function update_applications_list(applied_applications,open_applications){
    ...
    if(applied_applications[0]!=""){
        for(i=0;i<applied_applications.length;i++){
            _("applied_application_col").innerHTML+='<tr><td style="padding-right:50">'+applied_applications[i]+'  <a href="#!application_cancel_for['+applied_applications[i]+']" class="right btn grey text-white darken-1" style="">cancel</a></td></tr>'
        }
        //update_applications_list(applied_applications,"cancel")
    }
    ...
    _("open_application_col").innerHTML="";
    for(i=0;i<open_applications.length;i++){
        _("open_application_col").innerHTML+='<tr><td style="padding-right:50">'+open_applications[i]+'  <a href="#!apply_for['+open_applications[i]+']" class="right btn grey text-white darken-1" style="">apply</a></td></tr>'
    }
}
function modify_application(a){
    ...
    postData("http://127.0.0.1//upcjmi/server/exe.php?"+request_for,"application="+_("modal_header").innerHTML)
        .then(function(data) {
            if(data.data=="applied successfully"){
                _("application_modal_close").click()
                //window.location=""
                loading_scren_toggle()
...
function change_password(){
    if(_('new_password').value==_('confirm_new_password').value && _('new_password').value!=""&&_('current_password').value!="" && _("current_password").value!=_("new_password").value){
        data="current_password="+_("current_password").value+"&new_password="+_("new_password").value
        postData("http://127.0.0.1//upcjmi/server/exe.php?change_password",data)
        .then(function(data) {
            if(data.data=="Password changed"){
                alert("Password changed");window.location.hash="#!home";
                _("current_password").value=_("new_password").value=_('confirm_new_password').value="";
            }else{alert(data.data)}
        })
        .catch(error => console.error(error));
      }else if(_('new_password').value=="" || _("current_password").value==""){
        alert("enter Password")
      }else if(_("current_password").value==_("new_password").value){
        alert("NewPassword Can not be Current password")
      }else{
        alert("Confirm Password and new Password did not match")
        _('confirm_new_password').value=""
      }
